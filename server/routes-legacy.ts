...
export async function registerLegacyRoutes(
    ...
    try {
      ...
      if (employeeMasterRoute && employeeMasterRoute.isActive === false) {
        return res.json({
          ...
          message: "Please attach the employees data",
        });
      }
  ...
  // DELETE /api/attendance/hr/queries/batch/:batchId - HR: permanently delete a submission batch (same as attendance-verification.routes)
  app.delete(
  ...
  app.post("/api/admin/data-fetcher/sync-employees", requireAuth, requirePolicy("integrations.fetched-data.view"), async (req, res) => {
    try {
      ...
      if (!masterUrl) {
        return res.status(400).json({ 
          message: "Employee Master not configured. Please add 'Employee Master' in Data Sources (API Routing) and set it to Active." 
        });
      }
